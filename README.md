...
- **ライブラリ**: フォルダを指定してScanすると、配下の音源(wav/mp3/flac/aiff/ogg/m4a)を再帰的に解析してBPM・キー(Camelot表記併記)・エナジー(1-10)・波形を表示。解析結果はキャッシュされ、ファイルが変わらない限り再解析しません。
- **AI Mix Assistant**: ライブラリ項目をアームして「AI Mix Assistant」をONにすると、BPM/Camelotキー/エナジーの近さをローカルで採点し、次に繋ぐのに向いている曲順に並べ替え(クラウド通信・モデルDLなし)。
- **ホットキュー**: ライブラリの波形をクリックでキューポイントを追加(最大8個/曲)、右クリックで削除。キューのピルをクリックするとそのトラックをアーム+そのキュー位置から配置できます。
...
更新履歴は [PATCHNOTES.md](PATCHNOTES.md) を参照。
