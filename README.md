...
- **ライブラリ**: フォルダを指定してScanすると、配下の音源(wav/mp3/flac/aiff/ogg/m4a)を再帰的に解析してBPM・キー(Camelotコード付き)・Energy(1〜10)・波形を表示。解析結果はキャッシュされ、ファイルが変わらない限り再解析しません。
- **Hot Cue(rekordbox風)**: ライブラリの波形をクリックして試聴、丸ボタン(1〜8)でHot Cueを打ち込み保存。Cueをクリックするとその位置から再生 & そのトラックをアーム、そのままデッキに配置するとクリップがCue位置から始まります。右クリックで削除。Cueはファイルの再解析をまたいで保持されます。
- **Mix Assistant(ローカルAI)**: アーム中のトラックと相性の良い(Camelotキー・BPM・エネルギーの流れ)候補をライブラリから提案。ネットワーク通信のない、ブラウザ内で完結する軽量なヒューリスティック(ルールベース)処理です。
...
- BPM自動解析は目安です(特に単純なシンセ/パーカッション主体の音源だと数BPMずれることがあります)。Inspectorの「Source BPM」欄で手動修正できます。キー(Camelotコード)・Energyも同様に推定値であり、正確性を保証するものではありません。
- Mix Assistantはキー/BPM/Energyから算出したルールベースのスコアで、実際の楽曲の相性を保証するものではありません。
...
- `backend/` — FastAPI。`analysis.py`(BPM/キー/Camelot/Energy解析), `library.py`(フォルダScan+キャッシュ+Hot Cue永続化), `engine/`(タイムストレッチ・フィルター・リバーブ・ミックスダウン), `models.py`(プロジェクトのスキーマ), `main.py`(API。ライブラリ音源プレビュー配信・Hot Cue API含む。開発版/exe版でデータ保存先を自動切り替え)。
