...
- **Preview / Export**: PreviewはUI上部のプレイヤーで即再生確認。Export WAVでプロジェクト全体を1本のWAV(24bit/プロジェクトのサンプルレート)に書き出します。ルーラーをドラッグするとループ範囲を設定でき、その状態でPreviewすると区間だけをループ再生します(Ableton風のアレンジメントループ)。
- **保存/読込**: プロジェクトはJSONで保存され、一覧から読み込めます。
- **Undo/Redo**: `Ctrl+Z` / `Ctrl+Y`(またはトップバーのボタン)でほぼ全ての編集を取り消し・やり直しできます。
- **ハーモニックミックス支援**: ライブラリの各曲にCamelotキー表記を表示し、アーム/選択中の曲を基準に互換キーの曲をハイライト。検索・BPM/Key/Duration/Compatible★でのソートに対応(rekordboxの「関連曲」に近いローカル完結のルールベース処理)。
- **クリップボード**: クリップ選択中に `Ctrl+C`/`Ctrl+V`/`Ctrl+D` でコピー・貼り付け・複製。デッキヘッダーの「⧉」でデッキごと複製も可能。`Space`でPreview再生をトグル。
- **ライブラリ試聴デッキ + ホットキュー**: 曲をアームすると波形とプレイヤーが出て、タイムラインに置く前にソースファイルをそのまま試聴できます(rekordboxのブラウザプレビューに相当)。8個のホットキューパッドで再生位置を記憶し、数字キー`1`-`8`でジャンプ。キューは曲ごとに保存され、プロジェクトをまたいで使えます。
- **クレート**: ライブラリの曲を任意の名前のクレート(プレイリスト)にまとめて絞り込み表示。各曲の🗂ボタンから所属クレートをチェックボックスで切り替え。
- **VUメーター**: 上部Previewプレイヤーとライブラリ試聴デッキそれぞれに、Web Audio API(AnalyserNode)によるリアルタイムのレベルメーター(ピークホールド付き)を表示。
- **ビートジャンプ**: 試聴デッキに±1/±4拍のジャンプボタン(矢印キー / Shift+矢印キーでも操作可)。曲のBPMから拍単位で正確にスキップします(BPM不明の曲では無効化)。

詳細な変更履歴は [PATCHNOTES.md](PATCHNOTES.md) を参照してください。
