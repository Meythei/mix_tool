...
- **ライブラリ**: フォルダを指定してScanすると、配下の音源(wav/mp3/flac/aiff/ogg/m4a)を再帰的に解析してBPM・キー(Camelot表記つき)・エネルギー感・波形を表示。解析結果はキャッシュされ、ファイルが変わらない限り再解析しません。
- **Hot Cue**: ライブラリの波形をクリックしてキューポイントを打てます(rekordboxのホットキュー相当)。トラックに紐づいて保存され、クリップInspectorから該当トラックのキューへワンクリックでSrc offsetをジャンプできます。
- **Mix Assistant**: ライブラリで1曲アームして「🎯 Mix Assistant」を押すと、BPM・Camelotキーの相性でライブラリを並べ替えて次にかける曲を提案します。クラウドAIは使わない完全オフラインのルールベース機能です。
- **Beat Loop**: クリップInspectorの1/2/4/8/16/32拍ボタン・÷2/×2ボタンで、Src lengthを正確な拍数にワンクリックで合わせられます。
...
- `backend/` — FastAPI。`analysis.py`(BPM/キー/Camelot/エネルギー解析), `library.py`(フォルダScan+キャッシュ+Hot Cueのユーザーメタ), `engine/`(タイムストレッチ・フィルター・リバーブ・ミックスダウン), `models.py`(プロジェクトのスキーマ), `main.py`(API。開発版/exe版でデータ保存先を自動切り替え)。
- `CHANGELOG.md` — パッチノート。
